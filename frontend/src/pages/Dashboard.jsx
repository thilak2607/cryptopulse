    ...
    <div className="premium-card p-8 col-span-1 lg:col-span-1 border-gray-800 flex flex-col h-full">
      ...
      <div className="flex flex-col gap-4 flex-1">
        {symbols.map(symbol => {
           ...
           const target = suggestions[symbol]?.predictions?.[11]?.predicted_price || livePrice;
           return (
             <div key={symbol} className="flex flex-col gap-3 py-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors rounded-lg px-2">
               ...
               <div className="flex justify-between items-center">
                 ...
                 <div>
                    {getSignalBadge(suggestions[symbol]?.signal || '...')}
                  </div>
               </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios.get('https://cryptopulse-backend-k5bn.onrender.com/api/news')
      .then(res => {
